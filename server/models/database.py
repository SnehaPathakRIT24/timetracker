...
# native_enum=False makes Enum work with SQLite (stored as VARCHAR)
_enum_kw = dict(native_enum=False)
...
class TeamMember(Base):
    ...
    role = Column(Enum(RoleEnum, **_enum_kw), default=RoleEnum.member, nullable=False)
...
class ActivityRecord(Base):
    ...
    category = Column(Enum(CategoryEnum, **_enum_kw), default=CategoryEnum.unknown)
    confidence = Column(Float, default=0.0)
    manually_corrected = Column(Boolean, default=False)
    machine_id = Column(String(255), nullable=True)
    raw_data_hash = Column(String(64), nullable=True, index=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
...
class ClassificationRule(Base):
    ...
    field = Column(String(50), default="window_title")
    category = Column(Enum(CategoryEnum, **_enum_kw), nullable=False)
...
class CorrectionLog(Base):
    ...
    original_category = Column(Enum(CategoryEnum, **_enum_kw))
    corrected_category = Column(Enum(CategoryEnum, **_enum_kw), nullable=False)
